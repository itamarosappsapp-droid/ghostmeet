...
### 0. Push — только по прямой просьбе и только под релиз

**Коммить локально можно и нужно; отправлять на GitHub — нельзя, пока не попросили.** Push делается тогда, когда собран релиз, а не когда закончена правка.

Причина — в том, как проект раздаётся. Публичная история и есть витрина: каждый push немедленно виден, а фичи выпускаются поэтапно, а не сваливаются в репозиторий по мере написания. Владелец решает, что и когда становится публичным.

Что из этого следует по работе:

- **Готовая правка заканчивается локальным коммитом и словами «готово, отправлять?»**, а не отправкой.
- **CI до push не выполняется вовсе.** Единственная страховка на это время — полный прогон на своей машине, и он обязателен: `xcodebuild … test` целиком, с проверкой числа тестов, а не одного затронутого файла.
- Если между релизами накапливается несколько несвязанных фич, стоит спросить, не разложить ли их по веткам: коммиты, лежащие в `main` вперемешку, «поэтапно» уже не выпустишь — push отправит их одним куском.
