...
The MVP pipeline runs end to end: both channels are captured, turns are cut on pauses, speech is recognised locally into a live transcript — and a suggestion is generated when the user presses a chord, never on its own. **647 tests** across 87 suites (Swift Testing, target `GhostMeetTests`).
