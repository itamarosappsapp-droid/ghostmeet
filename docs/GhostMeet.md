...
5. **LLM**  
   ...
   Отмена рвёт и запрос к модели, и уже начатый снимок экрана. Перебитая подсказка переходит в состояние
   `superseded`: перестаёт расти, но остаётся в ленте. Распознавание перебитой реплики при этом
   не отменяется никогда — её слова нужны новому запросу, который уходит на полном вопросе.

   Ручной запрос — вопрос в поле ввода (`Ask`) или требование решить задачу с экрана (`Solve on screen`) —
   идёт по тому же пути и подчиняется тем же правилам: сначала перебивает то, что генерируется сейчас,
   потом снимает экран, потом стримит. Следующая реплика `Them` перебивает его так же, как автоматический
   ответ; речь `You` не отменяет и его. Параллельно с автоматическим циклом ручной запрос не идёт:
   он снимает признак «отвечаем на эту реплику», поэтому автоподсказка по уже закрытой реплике `Them`
   не сядет поверх того, что пользователь только что спросил.
...
`Assist` запускается сам, по закрытой реплике `Them`. `Ask` и `Solve on screen` — единственные, которые
запускает пользователь: первый из поля ввода в окне, второй кнопкой «Решить» и хоткеем `⌥⌘G`.
Окно транскрипта и бюджет токенов у каждого режима свои и берутся из `GhostMeet-Prompts.md`:
`Assist` — 12 реплик / 4k, `Ask` — 12 реплик / 2k, `Solve on screen` — транскрипт не читает вовсе / 4k.
...
│   ├── SuggestionComposer.swift  # движок не знает про режимы промптов: автоцикл и ручные запросы
...
│   │   ├── PromptFragments.swift     # общие блоки: экран (OCR), профиль
...
| Триггер подсказки | Хоткей | Автоматически, по паузе в речи собеседника; вручную — поле ввода и хоткей |
...
- [ ] Режимы Assist (автоматически) + What should I say + Ask и Solve on screen вручную, под теми же правилами отмены
- [ ] Профиль пользователя в system-промпте; несколько профилей с выбором активного и заполнением из резюме
