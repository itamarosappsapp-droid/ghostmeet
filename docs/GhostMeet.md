...
| **Подсказка подробно** | Тема незнакома целиком — развёрнутый разбор | `⌥⌘E` | экран + весь разговор |
| **Ask** | Свободный вопрос пользователя | поле ввода | экран + весь разговор |
| **Solve on screen** | Готовое решение задачи с экрана | `⌥⌘G` и кнопка | скриншот (+ OCR) |
...
- [ ] Скриншот в каждом запросе + OCR
- [ ] Два жанра подсказки по хоткею + `Ask` + `Solve on screen`, под общими правилами отмены
- [ ] Дозакрытие открытой реплики по нажатию с ожиданием распознавания
- [ ] Контекст собеседования поверх профиля; роды вопросов внутри промпта
