...
> **Состояние: 0.1.0.** MVP закрыт и пройден вживую на настоящем звонке. Это личный BYOK-инструмент,
...
- **Невидимость для шаринга.** Окно исключено из захвата через `sharingType = .none` — в том числе
  из наших собственных скриншотов, поэтому модель не видит свой предыдущий ответ.
...
флаг не снят. Подпись при этом настоящая и стабильная (Team ID `Z6F3T2TJVB`), и это важнее, чем
...
| [docs/adr/](docs/adr/) | десять архитектурных решений; заменённые остаются ради рассуждения, которое в них записано |
