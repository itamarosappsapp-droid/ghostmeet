...
struct BriefPromptTests {
    ...
    func theProhibitionsRestOnOneStatedReason() {
        ...
        #expect(
            system.contains("«Я не знаю», «данных мало», «сложно сказать» он вслух не произнесёт. Про заготовки и про то, чего нет в контексте, тоже молчи: это служебное, а не речь."),
    ...
    func inventingFactsAboutTheUserIsBannedInTheRules() {
        ...
        #expect(system.contains("Ни одного факта о пользователе, которого нет в контексте"))
        ...
        #expect(
            system.hasSuffix("и ни одного факта о нём, которого нет в контексте.**"),
    ...
    func thePronunciationRuleSurvives() {
        ...
        #expect(system.contains("есть в строке латиница, аббревиатура или формула — есть и скобка"))
    ...
    func thePronunciationExamplesCoverTheHardCases() {
        ...
        #expect(system.contains("min-heap (мин-хип)"))
        #expect(system.contains("TTL (ти-ти-эль)"), "аббревиатура, которую модель считала обиходной")
        #expect(system.contains("ZUNIONSTORE (зэт-юнион-стор)"), "идентификатор, не похожий на слово")
        #expect(system.contains("O(n log n) (о от эн лог эн)"), "нотацию надо разворачивать, а не транслитерировать")
    }

    @Test("Абсолютное правило огорожено от мусора, который оно само порождает")
    func thePronunciationRuleGuardsAgainstItsOwnGarbage() {
        let system = BriefPrompt.system(profile: .empty)

        // «с фича-флагом (feature flag)» — скобка была, содержимое вывернуто.
        #expect(system.contains("В скобке только русские буквы."))
        // «бакеты (бакеты)» — скобка на кириллице.
        #expect(system.contains("Термин, написанный кириллицей («постгрес», «кафка»), скобки не требует"))
        // «живой ecosystem (экосистема)», «рублей net (нет)» — латиница вставлена ради скобки.
        ...
        #expect(system.contains("В блоке кода скобок не ставь"))
    }
    ...
    func behaviouralQuestionsGetTheUsersOwnStory() {
        ...
        #expect(system.contains("«Был случай, когда мы с коллегой разошлись по решению"))
        #expect(system.contains("Сошлись на промежуточном варианте, и работать вместе это не помешало»"))
    }
    ...
    func everyBranchWorksWithNothingFilledIn() {
        ...
        #expect(system.contains("иначе строй костяк из его роли и стека"))
    ...
    func anEmptyTranscriptGetsAPlaceholder() {
        ...
        #expect(prompt.hasSuffix("Я уже говорю вслух. Продолжи за меня: напиши мою следующую фразу — ровно то, чего мне не хватает."))
    }
    ...
    func theClosingLineDoesNotMakeTheUserTheAddressee() {
        ...
        #expect(prompt.contains("напиши мою следующую фразу"))
    }
