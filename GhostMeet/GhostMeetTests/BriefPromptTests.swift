...
struct BriefPromptTests {
    ...
    func thePronunciationRuleGuardsAgainstItsOwnGarbage() {
        ...
        // «бакеты (бакеты)», «макротасков (макротасков)» — скобка на кириллице.
        // Проза тут прежняя, проверенная; к ней добавлен отрицательный образец,
        // потому что одной прозы в конце самого длинного абзаца промпта не
        // хватило: gemini-3.1-flash-lite выдал «Промис (промис)» при формально
        // запрещавшем это правиле.
        #expect(system.contains("Термин, написанный кириллицей («постгрес», «кафка»), скобки не требует"))
        #expect(system.contains("«бакеты (бакеты)» — мусор"))
        // Слово для образца выбрано такое, какого в ответе быть не может:
        // редакция с «промис (промис)» на живом прогоне вернула «Промис
        // (promise)» — модель прочитала пример как «у этого слова скобка бывает»
        // и подставила в неё оригинал латиницей.
        #expect(!system.contains("«промис (промис)»"))
        ...
        // «(мак-ро-таск)», «(и-вент луп)» — дефису из восемнадцати примеров
        // выше нужна граница, иначе он делит термин по слогам. Про это правило
        // молчало вовсе, зато показывало восемнадцать примеров через дефис.
        #expect(system.contains("дефис делит термин на слова, а не на слоги — «хеш-мапа», но не «(мак-ро-таск)»"))
