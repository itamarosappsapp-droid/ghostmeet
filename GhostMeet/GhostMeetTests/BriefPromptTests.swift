...
struct BriefPromptTests {
    ...
    func thePronunciationExamplesCoverTheHardCases() {
        ...
        #expect(system.contains("microtask queue (майкротаск-кью)"), "двусловный термин — одна скобка на оба слова, а не «queue (кью)» отдельно")
    ...
    func thePronunciationRuleGuardsAgainstItsOwnGarbage() {
        ...
        #expect(system.contains("В скобке русское произношение целиком"))
        ...
        #expect(system.contains("Скобку получает только латиница: написал по-русски — скобки нет вовсе"))
        #expect(system.contains("«микротаски (микротаски)» и «бакеты (бакеты)» мусор"))
        // «макро-таски (macro-tasks)» — скобка есть, а внутри оригинал вместо
        // произношения. Пять раз в одном ответе gemini-2.5-flash.
        #expect(system.contains("не оригинал латиницей"))
        // «microtask (майкротаск) queue (кью)» — термин разрезан пополам.
        #expect(system.contains("одну на весь термин целиком"))
        ...
        #expect(system.contains("не по слогам («мак-ро-таск»)"))
